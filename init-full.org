...
#+begin_src emacs-lisp :results silent
  ...
  (defun my-def-hide-prefix ()
    (interactive)
    (save-excursion
      (goto-char (point-min))
      (while (search-forward-regexp
              "^[[:space:]]*(\\(?:def[^[:space:]]*\\|comment\\)" nil t)
  ...
  (with-eval-after-load 'clojure-mode
    (define-key clojure-mode-map (kbd "C-c d h") #'my-def-hide-prefix)
    (define-key clojure-mode-map (kbd "C-c d s") #'my-def-show-prefix))
#+end_src

...
#+begin_src emacs-lisp :results silent
  ...
  (defun my-delete-trailing-whitespace ()
    (if my-delete-trailing-whitespace-on-save
        (delete-trailing-whitespace)))
  (add-hook 'before-save-hook 'my-delete-trailing-whitespace)
  (defun my-inhibit-delete-traling-whitespace ()
    (interactive)
    (set (make-local-variable 'my-delete-trailing-whitespace-on-save) nil))
#+end_src
...
#+begin_src emacs-lisp :results silent
  ...
  (my-global-set-key "C-c g o" 'ff-find-other-file)
  ...
  (my-global-set-key "C-c o b R" 'my-org-babel-remove-results)
  ...
  (my-global-set-key "C-c o L" 'my-org-insert-logbook-clocktable)
  ...
  (my-global-set-key "C-c C-w" 'toggle-word-wrap)
...
** Clojure Programming (inf-clojure)
#+begin_src emacs-lisp :results silent
...
#+begin_src emacs-lisp :results silent
  (use-package erlang
    :ensure t
    :pin melpa)
  (add-hook 'erlang-mode-hook (lambda () (setq font-lock-maximum-decoration t)))
#+end_src
...
#+begin_src emacs-lisp :results silent
  (defun my-cider-ns-refresh (arg)
    ...
    (if (null arg)
        (insert "(my/refresh)")
      (if (eq 4 (car arg))
          (insert "(my/refresh :all)")
        (insert "(my/refresh :nuke)")))
    ...
    (mapc (lambda (elt) (push elt prettify-symbols-alist))
          '(
            ...
            ("|]" . (?\s
                     (Bc . Bl) ?\]
                     (Br . Bc) ?\])))))
  (add-hook 'tuareg-mode-hook #'my-ocaml-prettify)
  (defun my-erlang-prettify ()
    (mapc (lambda (elt) (push elt prettify-symbols-alist))
          '(
            ("." . (?▮))
            ("=/=" . (?≡
            ...
            ("=>" . (?=
                     (Bc . Bl) ?=
                     (Br . Bc) ?≻)))))
  (add-hook 'erlang-mode-hook #'my-erlang-prettify)
...
** Org-babel remove result blocks
#+begin_src emacs-lisp :results silent
  (defun my-org-babel-remove-results ()
    (interactive)
    (org-babel-map-src-blocks nil (org-babel-remove-result)))
#+end_src

** Org insert logbook/clocktable
#+begin_src emacs-lisp :results silent
  (defun my-org-insert-logbook-clocktable (arg)
    (interactive "P")
    (if arg
        (insert "#+BEGIN: clocktable :scope subtree :maxlevel 10\n#+CAPTION: Clock summary at\n#+END:")
      (insert ":LOGBOOK:\n:END:")))
#+end_src
